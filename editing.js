...
class editable {
  ...
  constructor(target,container,containerWidth){
    ...
    this.store = {
              ...
              sx:1,
              sy:1,
              cx:this.targetBB.width/2,
              cy:this.targetBB.height/2,
              mat:''
    }

    this.initMat = `matrix(${this.store.sx * Math.cos(this.store.a)},${this.store.sy * Math.sin(this.store.a)},${-this.store.sx * Math.sin(this.store.a)},${this.store.sy * Math.cos(this.store.a)},${(-this.store.cx * Math.cos(this.store.a) + this.store.cy * Math.sin(this.store.a) + this.store.cx) * this.store.sx + this.store.tx},${(-this.store.cx * Math.sin(this.store.a) - this.store.cy * Math.cos(this.store.a) + this.store.cy) * this.store.sy + this.store.ty})`
  ...
  update(target,object){
    //crossbrowser
              ...
              this.store.tx = object.tx
              this.store.ty = object.ty
              this.store.a = object.a
              this.store.sx= object.sx
              this.store.sy= object.sy

              this.store.cx= object.cx
              this.store.cy= object.cy

    d3.select(target).style('transform',object.mat);
...
console.log(this.initMat)
  ...
  .style('transform','rotate(0deg) translate(0px,0px) scale(1,1)');  
...
function onTopLeftDrag(d){
        ...
        var sx = s;
        var sy = s;
        var tx = targetTransforms.tx;
        var ty = targetTransforms.ty;
        var a = targetTransforms.a;
        var cx =targetBB.width*s/2;
        var cy =targetBB.height*s/2;
        console.log(targetTransforms)
        var mat = `matrix(${sx * Math.cos(a)},${sy * Math.sin(a)},${-sx * Math.sin(a)},${sy * Math.cos(a)},${(-cx * Math.cos(a) + cy * Math.sin(a) + cx) * sx + tx},${(-cx * Math.sin(a) - cy * Math.cos(a) + cy) * sy + ty})`
...
// console.log(d3.event)
        ...
        // console.log()
         ...
         // console.log('topLeft',parseFloat(d3.select(`#${handleAttributes.ids.topLeft}`).attr('cx')),parseFloat(d3.select(`#${handleAttributes.ids.topLeft}`).attr('cy')));
         // console.log('bottomRight',parseFloat(d3.select(`#${handleAttributes.ids.bottomRight}`).attr('cx')),parseFloat(d3.select(`#${handleAttributes.ids.bottomRight}`).attr('cy')));
        ...
        var s = targetTransforms.scale.x;
        var tx = targetTransforms.translation.x;
        var ty = targetTransforms.translation.y;

          var imgRotx = ( (targetBB.width*s) + parseFloat(d3.select(`#${handleAttributes.ids.topLeft}`).attr('cx')) ) /2;
          var imgRoty = ( (targetBB.height*s) + parseFloat(d3.select(`#${handleAttributes.ids.bottomRight}`).attr('cy')) ) /2;

        var dSizeWidth = (targetBB.width*s) - targetBB.width;
        var dSizeHeight = (targetBB.height*s) - targetBB.height;
       console.log(s,tx,ty,dSizeWidth,dSizeHeight)
